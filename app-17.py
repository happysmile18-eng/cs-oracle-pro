...
def _patch_starlette_gzip_compatibility() -> None:
    """Protect Streamlit from a Starlette GZip API mismatch.

    Newer Starlette builds added a required ``thread_minimum_size`` keyword
    to ``GZipResponder``. Some Streamlit server releases still instantiate
    that responder with the older signature. Pinning Starlette in
    requirements.txt is the primary fix; this small patch is a second safety
    net in case the cloud environment resolves a newer build.
    """
    try:
        from starlette.middleware.gzip import GZipResponder

        parameter = inspect.signature(GZipResponder.__init__).parameters.get(
            "thread_minimum_size"
        )
        if parameter is None or parameter.default is not inspect.Parameter.empty:
            return

        original_init = GZipResponder.__init__
        ...
        GZipResponder.__init__ = compatible_init  # type: ignore[method-assign]
    except Exception:
        # The app must still start if Starlette is absent or changes again.
        pass


_patch_starlette_gzip_compatibility()
...
st.set_page_config(page_title="CS Oracle Drift-Aware Precision", page_icon="🎯", layout="wide", initial_sidebar_state="collapsed")
...
.team-rank-shell{border:1px solid var(--line);border-radius:20px;background:linear-gradient(145deg,#111620,#090c12);overflow:hidden;color:var(--ink)}
.team-rank-head{display:grid;grid-template-columns:54px minmax(180px,1.45fr) repeat(4,minmax(90px,.72fr));gap:.7rem;align-items:center;padding:.7rem .85rem;font-size:.68rem;text-transform:uppercase;letter-spacing:.12em;color:var(--muted);font-weight:850;background:#0a0e15}
.team-rank-shell + [data-testid="stVerticalBlock"]{gap:.42rem}
.profile-hero{color:var(--ink);padding:1.2rem 1.25rem;border:1px solid var(--line);border-radius:22px;background:radial-gradient(circle at 90% 10%,rgba(128,191,255,.18),transparent 38%),linear-gradient(145deg,#111826,#090c12);margin:.8rem 0 1rem}
.profile-name{font-size:clamp(2rem,5vw,3.4rem);font-weight:950;letter-spacing:-.055em;line-height:1;margin:.35rem 0}.profile-sub{color:var(--muted)}
.stat-chip{display:inline-block;padding:.38rem .62rem;border:1px solid #30394a;border-radius:999px;font-size:.78rem;margin:.25rem .28rem 0 0;color:#d8deea;background:#0c1119}
.result-strip{display:flex;gap:.35rem;flex-wrap:wrap;margin:.4rem 0 1rem}.result-dot{display:inline-flex;width:30px;height:30px;align-items:center;justify-content:center;border-radius:9px;font-weight:900}.result-win{background:rgba(97,221,162,.16);color:var(--green);border:1px solid rgba(97,221,162,.35)}.result-loss{background:rgba(255,89,107,.14);color:var(--red2);border:1px solid rgba(255,89,107,.32)}
@media(max-width:760px){.rank-row{grid-template-columns:38px 1fr 1fr}.hide-mobile{display:none}.block-container{padding-left:.7rem;padding-right:.7rem}.team-rank-head{grid-template-columns:38px minmax(145px,1.4fr) minmax(70px,.7fr) minmax(72px,.7fr)}.team-rank-hide-mobile{display:none}}
...
def canonicalise_team_identity(history: pd.DataFrame, upcoming: pd.DataFrame) -> tuple[pd.DataFrame, pd.DataFrame, dict[str, str], dict[str, int]]:
    """Unify renamed organisations by stable PandaScore team ID."""
    observations: list[dict[str, Any]] = []
    for frame in (history, upcoming):
        if frame.empty:
            continue
        for side in ("a", "b"):
            id_col, name_col = f"team_{side}_id", f"team_{side}"
            if id_col not in frame.columns or name_col not in frame.columns:
                continue
            for when, team_id, name in frame[["date", id_col, name_col]].itertuples(index=False, name=None):
                if safe_int(team_id, -1) >= 0 and str(name).strip():
                    observations.append({"date": parse_dt(when), "team_id": safe_int(team_id), "name": normalise_team(name)})
    if not observations:
        return history, upcoming, {}, {}
    obs = pd.DataFrame(observations).sort_values("date")
    canonical_by_id = obs.groupby("team_id", sort=False).tail(1).set_index("team_id")["name"].to_dict()
    alias_to_canonical: dict[str, str] = {}
    for row in obs.itertuples(index=False):
        alias_to_canonical[str(row.name)] = str(canonical_by_id.get(int(row.team_id), row.name))
    name_to_id = {str(name): int(team_id) for team_id, name in canonical_by_id.items()}
    ...
    def apply(frame: pd.DataFrame) -> pd.DataFrame:
        ...
        for side in ("a", "b"):
            id_col, name_col = f"team_{side}_id", f"team_{side}"
            if id_col in result.columns and name_col in result.columns:
                result[name_col] = result[id_col].map(canonical_by_id).fillna(result[name_col]).map(normalise_team)
        return result

    return apply(history), apply(upcoming), alias_to_canonical, name_to_id
...
class TeamState:
    ...
    # Glicko-style uncertainty. It rises during inactivity and falls as fresh,
    # high-quality evidence arrives. This prevents stale teams from looking certain.
    rating_deviation: float = 300.0
    volatility_ewma: float = 0.18
    reset_score: float = 0.0
    matches: int = 0
    last_date: pd.Timestamp | None = None
    recent: deque = field(default_factory=lambda: deque(maxlen=180))
    rating_points: deque = field(default_factory=lambda: deque(maxlen=240))
...
SIGNED_FEATURES = {
    ...
    "reliable_elo_diff", "certainty_diff", "reset_risk_diff", "volatility_diff",
    "trend_acceleration_diff", "quality_form_diff", "recent_sample_diff",
...
FEATURES = [
    "oracle_elo_diff", "core_elo_diff", "fast_elo_diff", "context_elo_diff", "format_elo_diff",
    "reliable_elo_diff", "certainty_diff", "uncertainty_sum", "reset_risk_diff", "reset_risk_sum",
    "volatility_diff", "trend_acceleration_diff", "quality_form_diff", "recent_sample_diff",
    "field_strength", "elo_momentum_30_diff", "elo_momentum_90_diff", "winrate_14_diff", "winrate_30_diff",
    "winrate_60_diff", "winrate_90_diff", "form5_diff", "form10_diff", "form20_diff",
    "performance_30_diff", "performance_90_diff", "opp_strength_diff", "margin_30_diff",
    "margin_90_diff", "sweep_win_diff", "sweep_loss_diff", "experience_diff", "rest_diff",
...
def effective_uncertainty(state: TeamState, when: pd.Timestamp) -> float:
    if state.last_date is None:
        return 350.0
    idle_days = max(0.0, (when - state.last_date).total_seconds() / 86400.0)
    # Glicko-like uncertainty growth during inactivity.
    return float(min(350.0, math.sqrt(state.rating_deviation ** 2 + 11.5 ** 2 * idle_days)))


def team_reset_risk(state: TeamState, when: pd.Timestamp) -> float:
    idle = rest_days(state, when)
    idle_risk = np.clip((idle - 35.0) / 95.0, 0.0, 0.52)
    core = regress_rating(state.core_elo, state.last_date, when, 520.0)
    fast = regress_rating(state.fast_elo, state.last_date, when, 110.0)
    divergence = np.clip(abs(fast - core) / 260.0, 0.0, 0.42)
    uncertainty = np.clip((effective_uncertainty(state, when) - 80.0) / 300.0, 0.0, 0.38)
    return float(np.clip(0.52 * state.reset_score + 0.25 * idle_risk + 0.15 * divergence + 0.08 * uncertainty, 0.0, 0.88))
...
def effective_components(state: TeamState, when: pd.Timestamp, best_of: int, lan: int) -> dict[str, float]:
    ...
    context = regress_rating(context_raw, state.last_date, when, 390.0)
    format_raw = state.bo1_elo if best_of == 1 else state.bo5_elo if best_of == 5 else state.bo3_elo
    format_elo = regress_rating(format_raw, state.last_date, when, 410.0)
    rd = effective_uncertainty(state, when)
    certainty = float(np.clip(1.0 - (rd - 45.0) / 305.0, 0.0, 1.0))
    reset_risk = team_reset_risk(state, when)
    # Current form matters more for volatile, inactive or probably rebuilt teams,
    # while stable teams preserve more long-run strength.
    fast_weight = float(np.clip(0.24 + 0.20 * reset_risk + 0.10 * (1.0 - certainty), 0.22, 0.48))
    context_weight = 0.12
    format_weight = 0.09
    core_weight = 1.0 - fast_weight - context_weight - format_weight
    oracle = core_weight * core + fast_weight * fast + context_weight * context + format_weight * format_elo
    reliable = 1500.0 + (oracle - 1500.0) * (0.72 + 0.28 * certainty)
    return {"core": core, "fast": fast, "context": context, "format": format_elo, "oracle": oracle,
            "reliable": reliable, "rd": rd, "certainty": certainty, "reset_risk": reset_risk,
...
def ranking_components(state: TeamState, when: pd.Timestamp) -> dict[str, float]:
    ...
    certainty = float(np.clip(1.0 - (rd - 45.0) / 305.0, 0.0, 1.0))
    reset_risk = team_reset_risk(state, when)
    fast_weight = float(np.clip(0.29 + 0.24 * reset_risk + 0.10 * (1.0 - certainty), 0.27, 0.58))
    oracle = (1.0 - fast_weight) * core + fast_weight * fast
    reliable = 1500.0 + (oracle - 1500.0) * (0.74 + 0.26 * certainty)
    return {"core": core, "fast": fast, "oracle": reliable, "raw_oracle": oracle,
            "rd": rd, "certainty": certainty, "reset_risk": reset_risk, "fast_weight": fast_weight}
...
def quality_form(state: TeamState, when: pd.Timestamp, days: int = 75) -> float:
    records = recent_records(state, when, days=days, last_n=24)
    if not records:
        return 0.0
    numerator, denominator = 0.0, 4.5
    for record in records:
        age = max(0.0, (when - record["date"]).total_seconds() / 86400.0)
        weight = 0.5 ** (age / 32.0)
        opponent_factor = float(np.clip((record.get("opponent_elo", 1500.0) - 1350.0) / 450.0, 0.25, 1.35))
        value = 0.72 * safe_float(record.get("performance"), 0.0) + 0.28 * (safe_float(record.get("result"), 0.5) - 0.5)
        numerator += weight * opponent_factor * value
        denominator += weight
    return float(np.clip(numerator / denominator, -0.9, 0.9))


def recent_sample(state: TeamState, when: pd.Timestamp, days: int = 90) -> int:
    return len(recent_records(state, when, days=days))


def trend_acceleration(state: TeamState, when: pd.Timestamp) -> float:
    return rating_momentum(state, when, 30) - 0.42 * rating_momentum(state, when, 90)
...
def build_features(states: dict[str, TeamState], h2h: dict[tuple[str, str], deque], when: pd.Timestamp, team_a: str, team_b: str, best_of: int, lan: int, tier: float, event: str = "") -> dict[str, float]:
    ...
    field_strength = float(np.clip((((ca["oracle"] + cb["oracle"]) / 2.0) - 1325.0) / 520.0, 0.0, 1.0))
    return {
        ...
        "format_elo_diff": ca["format"] - cb["format"], "reliable_elo_diff": ca["reliable"] - cb["reliable"],
        "certainty_diff": ca["certainty"] - cb["certainty"], "uncertainty_sum": (ca["rd"] + cb["rd"]) / 700.0,
        "reset_risk_diff": ca["reset_risk"] - cb["reset_risk"], "reset_risk_sum": ca["reset_risk"] + cb["reset_risk"],
        "volatility_diff": a.volatility_ewma - b.volatility_ewma,
        "trend_acceleration_diff": trend_acceleration(a, when) - trend_acceleration(b, when),
        "quality_form_diff": quality_form(a, when) - quality_form(b, when),
        "recent_sample_diff": float(math.log1p(recent_sample(a, when)) - math.log1p(recent_sample(b, when))),
        ...
        "streak_diff": float(a.streak - b.streak), "event_form_diff": event_a - event_b,
        ...
        "sample_min": float(min(a.matches, b.matches)), "best_of": float(best_of), "lan": float(lan), "tier": float(tier),
    }
...
def update_ratings(states: dict[str, TeamState], h2h: dict[tuple[str, str], deque], ledger: list[dict[str, Any]], when: pd.Timestamp, team_a: str, team_b: str, a_win: int, score_a: int, score_b: int, best_of: int, lan: int, tier: float, event: str) -> None:
    ...
    expected_a = logistic_elo(a_comp["reliable"] - b_comp["reliable"], scale=390.0)
    needed = 1 if best_of == 1 else best_of // 2 + 1
    map_share_a = score_a / max(1.0, score_a + score_b)
    signed_margin = (score_a - score_b) / needed
    margin_multiplier = 1.0 + 0.17 * math.log1p(abs(score_a - score_b)) + (0.055 if score_b == 0 or score_a == 0 else 0.0)
    field_strength = float(np.clip((((a_comp["oracle"] + b_comp["oracle"]) / 2.0) - 1325.0) / 520.0, 0.0, 1.0))
    effective_tier = float(np.clip(0.58 * tier + 0.42 * field_strength, 0.25, 1.0))
    tier_multiplier = 0.82 + 0.43 * effective_tier
    experience_multiplier = 1.30 - 0.40 * min(1.0, min(a.matches, b.matches) / 75.0)
    uncertainty_multiplier = 0.92 + 0.32 * (1.0 - min(a_comp["certainty"], b_comp["certainty"]))
    base_k = 25.5 * tier_multiplier * experience_multiplier * uncertainty_multiplier * margin_multiplier
    residual = a_win - expected_a
    core_delta = base_k * residual
    fast_delta = (52.0 * tier_multiplier * margin_multiplier * uncertainty_multiplier) * residual
    context_delta = (34.0 * tier_multiplier * margin_multiplier) * residual
    format_delta = (31.0 * tier_multiplier * margin_multiplier) * residual
    ...
    performance_a = 0.74 * residual + 0.26 * (map_share_a - 0.5)
    alpha = 0.14
    ...
    a.margin_ewma = 0.83 * a.margin_ewma + 0.17 * signed_margin
    b.margin_ewma = 0.83 * b.margin_ewma - 0.17 * signed_margin
    a.performance_ewma = 0.83 * a.performance_ewma + 0.17 * performance_a
    b.performance_ewma = 0.83 * b.performance_ewma - 0.17 * performance_a

    surprise = float(np.clip(abs(residual) + 0.18 * abs(signed_margin), 0.0, 1.0))
    a.volatility_ewma = 0.88 * a.volatility_ewma + 0.12 * abs(performance_a)
    b.volatility_ewma = 0.88 * b.volatility_ewma + 0.12 * abs(performance_a)
    # Ordinary match randomness should not look like a roster reset. Only the
    # excess surprise above a normal baseline accumulates as structural-change risk.
    structural_shock = max(0.0, surprise - 0.40) / 0.60
    a.reset_score = float(np.clip(0.92 * a.reset_score + 0.08 * structural_shock, 0.0, 1.0))
    b.reset_score = float(np.clip(0.92 * b.reset_score + 0.08 * structural_shock, 0.0, 1.0))
    information = (0.82 + 0.55 * effective_tier) * (1.0 + 0.12 * (best_of >= 3))
    a.rating_deviation = float(max(48.0, a_comp["rd"] / math.sqrt(1.0 + 0.24 * information)))
    b.rating_deviation = float(max(48.0, b_comp["rd"] / math.sqrt(1.0 + 0.24 * information)))
    ...
    record_a = {"date": when, "result": float(a_win), "expected": expected_a, "performance": performance_a,
                ...
                "tier": effective_tier, "event_key": normalise_event(event), "sweep_win": float(a_win == 1 and score_b == 0 and best_of >= 3),
                "sweep_loss": float(a_win == 0 and score_a == 0 and best_of >= 3), "field_strength": field_strength}
    record_b = {"date": when, "result": float(1 - a_win), "expected": 1 - expected_a, "performance": -performance_a,
                ...
                "tier": effective_tier, "event_key": normalise_event(event), "sweep_win": float(a_win == 0 and score_a == 0 and best_of >= 3),
                "sweep_loss": float(a_win == 1 and score_b == 0 and best_of >= 3), "field_strength": field_strength}
    ...
    a.rating_points.append({"date": when, "oracle": rank_a, "core": a.core_elo, "fast": a.fast_elo,
                            "certainty": 100 * ranking_components(a, when)["certainty"]})
    b.rating_points.append({"date": when, "oracle": rank_b, "core": b.core_elo, "fast": b.fast_elo,
                            "certainty": 100 * ranking_components(b, when)["certainty"]})

    common = {"date": when, "event": event, "best_of": best_of, "lan": lan, "tier": effective_tier,
    ...
    ledger.append({**common, "team": team_a, "opponent": team_b, "result": "W" if a_win else "L",
                   "score": f"{score_a}-{score_b}", "expected": expected_a, "before": a_comp["oracle"], "after": after_a,
                   "delta": after_a - a_comp["oracle"], "uncertainty": a.rating_deviation, "reset_risk": team_reset_risk(a, when)})
    ledger.append({**common, "team": team_b, "opponent": team_a, "result": "L" if a_win else "W",
                   "score": f"{score_b}-{score_a}", "expected": 1 - expected_a, "before": b_comp["oracle"], "after": after_b,
                   "delta": after_b - b_comp["oracle"], "uncertainty": b.rating_deviation, "reset_risk": team_reset_risk(b, when)})
...
def make_training_data(history: pd.DataFrame) -> TrainingData:
    ...
    for row in history.sort_values("date").itertuples(index=False):
        ...
        sample_quality = 0.76 + 0.24 * min(1.0, features["sample_min"] / 22.0)
        uncertainty_quality = 1.0 - 0.12 * min(1.0, features["uncertainty_sum"])
        format_quality = 0.91 if int(row.best_of) == 1 else 1.03 if int(row.best_of) == 5 else 1.0
        base_weight = quality_weight * sample_quality * uncertainty_quality * format_quality
    ...
    winner_weights = np.asarray(base_weights, dtype=float)
    score_weights = np.asarray([base_weights[i] * class_weights[labels[i]] for i in range(len(labels))], dtype=float)
    return TrainingData(pd.DataFrame(x_rows, columns=FEATURES), labels, np.asarray(dates, dtype="datetime64[ns]"), winner_weights,
                        score_weights, np.asarray(originals, dtype=bool), states, h2h, pd.DataFrame(ledger))


WINNER_MODEL_CONFIGS = [
    {"name": "Ultra-recent 9-month", "half_life": 92.0, "window_days": 285, "n_estimators": 500, "max_depth": 2, "learning_rate": 0.028, "min_child_weight": 10.0, "subsample": 0.92, "colsample_bytree": 0.90, "reg_alpha": 0.45, "reg_lambda": 6.0, "gamma": 0.025},
    {"name": "Fast current-form", "half_life": 125.0, "window_days": 420, "n_estimators": 520, "max_depth": 2, "learning_rate": 0.027, "min_child_weight": 9.5, "subsample": 0.93, "colsample_bytree": 0.90, "reg_alpha": 0.40, "reg_lambda": 5.7, "gamma": 0.025},
    {"name": "Balanced temporal", "half_life": 205.0, "window_days": 700, "n_estimators": 560, "max_depth": 2, "learning_rate": 0.026, "min_child_weight": 9.0, "subsample": 0.93, "colsample_bytree": 0.89, "reg_alpha": 0.34, "reg_lambda": 5.4, "gamma": 0.022},
    {"name": "Stable long-memory", "half_life": 390.0, "window_days": 1000, "n_estimators": 620, "max_depth": 3, "learning_rate": 0.022, "min_child_weight": 12.0, "subsample": 0.90, "colsample_bytree": 0.84, "reg_alpha": 0.58, "reg_lambda": 7.0, "gamma": 0.065},
    {"name": "BO1 upset specialist", "half_life": 165.0, "window_days": 720, "focus": "bo1", "n_estimators": 520, "max_depth": 2, "learning_rate": 0.027, "min_child_weight": 10.0, "subsample": 0.92, "colsample_bytree": 0.88, "reg_alpha": 0.48, "reg_lambda": 6.2, "gamma": 0.035},
    {"name": "BO3 reliability specialist", "half_life": 220.0, "window_days": 820, "focus": "bo3", "n_estimators": 570, "max_depth": 3, "learning_rate": 0.024, "min_child_weight": 10.5, "subsample": 0.92, "colsample_bytree": 0.87, "reg_alpha": 0.44, "reg_lambda": 6.3, "gamma": 0.050},
    {"name": "Elite-event specialist", "half_life": 245.0, "window_days": 900, "focus": "high_tier", "n_estimators": 560, "max_depth": 2, "learning_rate": 0.025, "min_child_weight": 9.5, "subsample": 0.92, "colsample_bytree": 0.88, "reg_alpha": 0.38, "reg_lambda": 5.8, "gamma": 0.030},
    {"name": "Lower-tier noise specialist", "half_life": 150.0, "window_days": 500, "focus": "low_tier", "n_estimators": 520, "max_depth": 2, "learning_rate": 0.026, "min_child_weight": 12.0, "subsample": 0.90, "colsample_bytree": 0.86, "reg_alpha": 0.62, "reg_lambda": 7.2, "gamma": 0.065},
    {"name": "LAN specialist", "half_life": 260.0, "window_days": 950, "focus": "lan", "n_estimators": 540, "max_depth": 2, "learning_rate": 0.025, "min_child_weight": 10.5, "subsample": 0.91, "colsample_bytree": 0.88, "reg_alpha": 0.45, "reg_lambda": 6.2, "gamma": 0.040},
]


GATE_COLUMNS = [
    "model_confidence", "entropy", "elo_confidence", "model_elo_agree", "model_elo_gap",
    "symmetry_quality", "member_agreement", "sample_quality", "certainty_quality", "reset_quality",
    "tier", "field_strength", "abs_oracle_elo", "abs_reliable_elo", "abs_fast_elo",
    "abs_momentum_30", "abs_trend_accel", "abs_form_30", "abs_quality_form", "rest_abs", "best_of",
]


def new_winner_model(config: dict[str, Any], seed: int = 27) -> xgb.XGBClassifier:
    return xgb.XGBClassifier(
        **{key: value for key, value in config.items() if key not in {"name", "half_life", "window_days", "focus"}},
...
def gate_frame(features: pd.DataFrame, p: np.ndarray, elo_p: np.ndarray, symmetry_gap: np.ndarray, member_spread: np.ndarray | None = None) -> pd.DataFrame:
    p = np.asarray(p, dtype=float)
    elo_p = np.asarray(elo_p, dtype=float)
    member_spread = np.zeros_like(p) if member_spread is None else np.asarray(member_spread, dtype=float)
    entropy = -(p * np.log(np.clip(p, 1e-9, 1)) + (1 - p) * np.log(np.clip(1 - p, 1e-9, 1))) / math.log(2)
    return pd.DataFrame({
        "model_confidence": np.maximum(p, 1 - p), "entropy": entropy,
        "elo_confidence": np.maximum(elo_p, 1 - elo_p),
        "model_elo_agree": ((p >= 0.5) == (elo_p >= 0.5)).astype(float),
        "model_elo_gap": np.abs(p - elo_p),
        "symmetry_quality": 1.0 - np.clip(symmetry_gap / 0.20, 0.0, 1.0),
        "member_agreement": 1.0 - np.clip(member_spread / 0.16, 0.0, 1.0),
        "sample_quality": np.clip(features["sample_min"].to_numpy(dtype=float) / 38.0, 0.0, 1.0),
        "certainty_quality": 1.0 - np.clip(features["uncertainty_sum"].to_numpy(dtype=float), 0.0, 1.0),
        "reset_quality": 1.0 - np.clip(features["reset_risk_sum"].to_numpy(dtype=float) / 1.2, 0.0, 1.0),
        "tier": features["tier"].to_numpy(dtype=float), "field_strength": features["field_strength"].to_numpy(dtype=float),
        "abs_oracle_elo": np.abs(features["oracle_elo_diff"].to_numpy(dtype=float)) / 250.0,
        "abs_reliable_elo": np.abs(features["reliable_elo_diff"].to_numpy(dtype=float)) / 250.0,
        ...
        "abs_trend_accel": np.abs(features["trend_acceleration_diff"].to_numpy(dtype=float)) / 140.0,
        "abs_form_30": np.abs(features["winrate_30_diff"].to_numpy(dtype=float)) / 0.35,
        "abs_quality_form": np.abs(features["quality_form_diff"].to_numpy(dtype=float)) / 0.35,
...
def temporal_weights(training: TrainingData, mask: np.ndarray, reference_date: np.datetime64 | pd.Timestamp, half_life: float, score: bool = False) -> np.ndarray:
    ...
    source = training.score_weights if score else training.weights
    return np.asarray(source[mask], dtype=float) * np.asarray(decay, dtype=float)


def specialist_weights(frame: pd.DataFrame, focus: str | None) -> np.ndarray:
    weights = np.ones(len(frame), dtype=float)
    if not focus:
        return weights
    bo = frame["best_of"].to_numpy(float)
    tier = frame["tier"].to_numpy(float)
    lan = frame["lan"].to_numpy(float)
    if focus == "bo1":
        weights *= np.where(bo == 1, 2.35, 0.58)
    elif focus == "bo3":
        weights *= np.where(bo == 3, 1.65, 0.68)
    elif focus == "high_tier":
        weights *= np.where(tier >= 0.70, 2.00, 0.58)
    elif focus == "low_tier":
        weights *= np.where(tier < 0.70, 1.55, 0.62)
    elif focus == "lan":
        weights *= np.where(lan >= 0.5, 2.05, 0.62)
    return weights
...
def fit_winner_candidate(training: TrainingData, mask: np.ndarray, config: dict[str, Any], reference_date: Any, seed: int) -> xgb.XGBClassifier:
    ...
    local_mask = np.asarray(mask, dtype=bool).copy()
    window_days = float(config.get("window_days", 0) or 0)
    if window_days > 0:
        ref = pd.Timestamp(reference_date)
        dates = pd.to_datetime(training.dates)
        window_mask = dates >= ref - pd.Timedelta(days=window_days)
        proposed = local_mask & np.asarray(window_mask, dtype=bool)
        if proposed.sum() >= 260:
            local_mask = proposed
    frame = training.x.loc[local_mask, FEATURES]
    weights = temporal_weights(training, local_mask, reference_date, float(config["half_life"]))
    weights *= specialist_weights(frame, config.get("focus"))
    model.fit(frame, y[local_mask], sample_weight=weights)
    return model
...
def heuristic_probabilities(frame: pd.DataFrame) -> np.ndarray:
    score = (
        frame["reliable_elo_diff"].to_numpy(float) / 235.0
        + 1.18 * frame["winrate_30_diff"].to_numpy(float)
        + 0.62 * frame["winrate_90_diff"].to_numpy(float)
        + 1.05 * frame["quality_form_diff"].to_numpy(float)
        + 0.82 * frame["performance_30_diff"].to_numpy(float)
        + 0.28 * frame["margin_30_diff"].to_numpy(float)
        + frame["trend_acceleration_diff"].to_numpy(float) / 135.0
        + 0.28 * frame["h2h_diff"].to_numpy(float)
        + 0.58 * frame["event_form_diff"].to_numpy(float)
        + 0.28 * frame["high_tier_form_diff"].to_numpy(float)
        + 0.22 * frame["favorite_conversion_diff"].to_numpy(float)
        + 0.16 * frame["close_match_diff"].to_numpy(float)
        + 0.22 * frame["consistency_diff"].to_numpy(float)
        + 0.15 * frame["certainty_diff"].to_numpy(float)
        - 0.34 * frame["reset_risk_diff"].to_numpy(float)
        - 0.17 * frame["fatigue_diff"].to_numpy(float)
        - 0.14 * frame["rust_diff"].to_numpy(float)
    )
...
def stack_design(base_matrix: np.ndarray, frame: pd.DataFrame) -> np.ndarray:
    ...
    elo = np.asarray([logistic_elo(value, scale=390.0) for value in frame["reliable_elo_diff"].to_numpy(float)])
    fast = np.asarray([logistic_elo(0.72 * value + 0.28 * core, scale=390.0) for value, core in zip(frame["fast_elo_diff"], frame["core_elo_diff"])])
    ...
    certainty_quality = 1.0 - np.clip(frame["uncertainty_sum"].to_numpy(float), 0.0, 1.0)
    reset_quality = 1.0 - np.clip(frame["reset_risk_sum"].to_numpy(float) / 1.2, 0.0, 1.0)
    tier = frame["tier"].to_numpy(float); field_strength = frame["field_strength"].to_numpy(float)
    bo1 = (frame["best_of"].to_numpy(float) == 1).astype(float)
    bo3 = (frame["best_of"].to_numpy(float) == 3).astype(float)
    bo5 = (frame["best_of"].to_numpy(float) >= 5).astype(float)
    lan = frame["lan"].to_numpy(float)
    abs_elo = np.abs(frame["reliable_elo_diff"].to_numpy(float)) / 400.0
    event_edge = frame["event_form_diff"].to_numpy(float)
    consistency = frame["consistency_diff"].to_numpy(float)
    quality_edge = frame["quality_form_diff"].to_numpy(float)
    trend_edge = frame["trend_acceleration_diff"].to_numpy(float) / 160.0
    mean_logit = np.asarray([logit(value) for value in base_mean])
    columns.extend([
        np.asarray([logit(value) for value in elo]), np.asarray([logit(value) for value in fast]),
        np.asarray([logit(value) for value in form]), mean_logit, base_spread,
        sample_quality, certainty_quality, reset_quality, tier, field_strength,
        bo1, bo3, bo5, lan, abs_elo, event_edge, consistency, quality_edge, trend_edge,
        mean_logit * sample_quality, mean_logit * certainty_quality, mean_logit * reset_quality,
        mean_logit * tier, mean_logit * field_strength,
        mean_logit * (1.0 - np.minimum(base_spread * 5.0, 1.0)),
    ])
...
def train_bundle_cached(history_json: str) -> ModelBundle:
    ...
    selected_configs = WINNER_MODEL_CONFIGS[1:5]
    ...
    if len(history) >= MIN_MODEL_MATCHES:
        ...
        if train_mask.sum() >= 300 and len(calib_indices) >= 95 and test_mask.sum() >= 80:
            ...
            chronological_folds = [fold for fold in np.array_split(np.arange(len(stack_idx)), 3) if len(fold) >= 12]
            for candidate_i, config in enumerate(WINNER_MODEL_CONFIGS):
                ...
                recent_penalty = fold_scores[-1] if fold_scores else float(brier_score_loss(stack_y, p))
                score = float(np.mean(fold_scores) + 0.28 * np.std(fold_scores) + 0.18 * recent_penalty) if fold_scores else float(brier_score_loss(stack_y, p))
                candidate_rows.append((score, config, model))
            candidate_rows.sort(key=lambda item: item[0])
            selected_configs = [item[1] for item in candidate_rows[:4]]
            candidate_models = [(item[1], item[2]) for item in candidate_rows[:4]]
            ...
            calib_p, calib_gap, calib_base = engine_probs(eval_engine, calib_x)
            calib_elo_p = np.asarray([logistic_elo(value, scale=390.0) for value in calib_x["reliable_elo_diff"]], dtype=float)
            calib_gate_x = gate_frame(calib_x, calib_p, calib_elo_p, calib_gap, np.std(calib_base, axis=1))
            ...
            if gate_split >= 35 and len(np.unique(calib_correct[:gate_split])) >= 2:
                gate_candidates: list[tuple[str, Any]] = [
                    ("Logistic reliability gate", make_pipeline(StandardScaler(), LogisticRegression(C=0.30, max_iter=1800, class_weight="balanced"))),
                    ("Shallow boosted reliability gate", xgb.XGBClassifier(n_estimators=130, max_depth=2, learning_rate=0.035,
                        min_child_weight=8.0, subsample=0.88, colsample_bytree=0.88, reg_alpha=0.65, reg_lambda=7.0,
                        gamma=0.08, objective="binary:logistic", eval_metric="logloss", random_state=144, n_jobs=2, tree_method="hist")),
                ]
                gate_results = []
                for gate_name, candidate in gate_candidates:
                    candidate.fit(calib_gate_x.iloc[:gate_split], calib_correct[:gate_split])
                    valid_scores = candidate.predict_proba(calib_gate_x.iloc[gate_split:])[:, 1]
                    threshold, validation = choose_precision_threshold(valid_scores, calib_correct[gate_split:])
                    rank_score = (1 if validation.get("target_met") else 0, validation.get("accuracy", 0.0), validation.get("coverage", 0.0))
                    gate_results.append((rank_score, gate_name, candidate, threshold, validation))
                _, gate_name, gate_model, gate_threshold, gate_validation = max(gate_results, key=lambda item: item[0])
                gate_validation = {**gate_validation, "model": gate_name}
            ...
            if len(score_train_labels) >= 4 and score_train_mask.sum() >= 280:
                ...
                eval_score_model.fit(training.x.loc[score_train_mask, FEATURES], score_y_train,
                                     sample_weight=temporal_weights(training, score_train_mask, train_cut, 235.0, score=True))
            ...
            test_p, test_gap, test_base = engine_probs(eval_engine, test_x)
            test_pred = (test_p >= 0.5).astype(int)
            test_elo_p = np.asarray([logistic_elo(value, scale=390.0) for value in test_x["reliable_elo_diff"]], dtype=float)
            test_gate_x = gate_frame(test_x, test_p, test_elo_p, test_gap, np.std(test_base, axis=1))
            ...
            quality_mask = ((test_x["sample_min"].to_numpy(float) >= 16) &
                            (test_x["reset_risk_sum"].to_numpy(float) <= 0.72) &
                            (test_x["uncertainty_sum"].to_numpy(float) <= 0.72) &
                            (test_x["best_of"].to_numpy(float) >= 3))
            strong_mask = quality_mask & (gate_scores >= np.quantile(gate_scores, 0.72))
            metrics = {
                ...
                "quality_match_accuracy": float(accuracy_score(test_y[quality_mask], test_pred[quality_mask])) if quality_mask.any() else 0.0,
                "quality_match_count": int(quality_mask.sum()),
                "strong_signal_accuracy": float(accuracy_score(test_y[strong_mask], test_pred[strong_mask])) if strong_mask.any() else 0.0,
        ...
        if len(all_score_labels) >= 4:
            ...
            final_score.fit(training.x[FEATURES], score_y_all,
                            sample_weight=temporal_weights(training, full_mask, final_ref, 235.0, score=True))
...
def winner_reliability(bundle: ModelBundle, features: dict[str, float]) -> dict[str, float]:
    ...
    if bundle.winner_model is None:
        p = logistic_elo(features.get("reliable_elo_diff", features["oracle_elo_diff"]), scale=390.0)
    ...
    elo_p = np.asarray([logistic_elo(features["reliable_elo_diff"], scale=390.0)])
    gate_x = gate_frame(frame, calibrated, elo_p, gap, np.std(base, axis=1))
...
def elo_table(bundle: ModelBundle) -> pd.DataFrame:
    ...
    for team, state in bundle.states.items():
        ...
        rows.append({"Team": team, "Oracle Elo": components["oracle"], "Core": components["core"], "Fast": components["fast"],
                     "Certainty": 100 * components["certainty"], "Reset risk": 100 * components["reset_risk"],
                     "Fast weight": 100 * components["fast_weight"],
...
def elo_chart(bundle: ModelBundle, team: str, height: int = 390, compact: bool = False) -> go.Figure:
    ...
    if not series.empty:
        ...
        if "certainty" in series.columns and not compact:
            fig.add_trace(go.Scatter(x=series["date"], y=series["certainty"], mode="lines", name="Rating certainty", line=dict(width=1.4, dash="dashdot"), yaxis="y2"))
            fig.update_layout(yaxis2=dict(title="Certainty", overlaying="y", side="right", range=[0, 100], ticksuffix="%", showgrid=False))
        peak_i = int(series["oracle"].idxmax())
        peak = series.loc[peak_i]
        fig.add_trace(go.Scatter(x=[peak["date"]], y=[peak["oracle"]], mode="markers+text", text=["Peak"], textposition="top center", name="Peak", marker=dict(size=10)))
...
def rating_profile_chart(state: TeamState, when: pd.Timestamp) -> go.Figure:
    values = {
        "Core": regress_rating(state.core_elo, state.last_date, when, 520.0),
        "Fast": regress_rating(state.fast_elo, state.last_date, when, 125.0),
        "LAN": regress_rating(state.lan_elo, state.last_date, when, 420.0),
        "Online": regress_rating(state.online_elo, state.last_date, when, 420.0),
        "BO1": regress_rating(state.bo1_elo, state.last_date, when, 440.0),
        "BO3": regress_rating(state.bo3_elo, state.last_date, when, 440.0),
        "BO5": regress_rating(state.bo5_elo, state.last_date, when, 440.0),
    }
    fig = go.Figure(go.Bar(x=list(values.values()), y=list(values.keys()), orientation="h", text=[f"{v:.0f}" for v in values.values()], textposition="auto"))
    fig.update_layout(height=340, margin=dict(l=8, r=8, t=24, b=8), xaxis_title="Elo", yaxis_title=None, showlegend=False)
    return fig


def format_context_table(state: TeamState, when: pd.Timestamp) -> pd.DataFrame:
    rows: list[dict[str, Any]] = []
    contexts = [("BO1", {"best_of": 1}), ("BO3", {"best_of": 3}), ("BO5", {"best_of": 5}), ("LAN", {"lan": 1}), ("Online", {"lan": 0})]
    for label, filters in contexts:
        records = recent_records(state, when, days=365, **filters)
        if records:
            wins = sum(record["result"] for record in records)
            rows.append({"Context": label, "Matches": len(records), "Win rate": 100 * wins / len(records), "Opponent Elo": float(np.mean([record["opponent_elo"] for record in records]))})
        else:
            rows.append({"Context": label, "Matches": 0, "Win rate": 50.0, "Opponent Elo": 1500.0})
    return pd.DataFrame(rows)


def recent_results_html(bundle: ModelBundle, team: str, count: int = 12) -> str:
    history = team_history_frame(bundle, team).tail(count)
    if history.empty:
        return '<span class="muted">No recent results</span>'
    chips = []
    for result in history["result"].tolist():
        css = "result-win" if result == "W" else "result-loss"
        chips.append(f'<span class="result-dot {css}">{result}</span>')
    return '<div class="result-strip">' + ''.join(chips) + '</div>'
...
def render_clickable_elo_list(frame: pd.DataFrame, limit: int = 30) -> str | None:
    if frame.empty:
        st.info("No teams match this ranking filter.")
        return None
    clicked: str | None = None
    st.markdown('<div class="team-rank-shell"><div class="team-rank-head"><div>#</div><div>Team · click to inspect</div><div>Elo</div><div>Fast</div><div class="team-rank-hide-mobile">30d</div><div class="team-rank-hide-mobile">Heat</div></div></div>', unsafe_allow_html=True)
    for _, row in frame.head(limit).iterrows():
        with st.container(border=True):
            c0, c1, c2, c3, c4, c5 = st.columns([.42, 2.2, .78, .78, .78, .72])
            c0.markdown(f"<div class='rank-n'>{int(row['Rank'])}</div>", unsafe_allow_html=True)
            label = f"{row['Team']}  ·  {row['Form']}"
            if c1.button(label, key=stable_key("open_team", row["Team"]), use_container_width=True, type="primary" if st.session_state.get("elo_selected_team") == row["Team"] else "secondary"):
                clicked = str(row["Team"])
            c2.markdown(f"**{float(row['Oracle Elo']):.0f}**")
            c3.markdown(f"{float(row['Fast']):.0f}")
            c4.markdown(f"{float(row['30d change']):+.0f}")
            c5.markdown(f"{float(row['Heat']):.0f}/100")
    return clicked


def team_profile_panel(bundle: ModelBundle, rankings: pd.DataFrame, team: str, upcoming: pd.DataFrame, token: str, team_id_lookup: dict[str, int], display_timezone: str) -> None:
    if team not in bundle.states:
        st.warning("That team is no longer present in the current rating database.")
        return
    state = bundle.states[team]
    now = pd.Timestamp.now(tz="UTC")
    current = ranking_components(state, now)
    form = hot_form_metrics(state, now)
    rank_rows = rankings[rankings["Team"] == team]
    rank = int(rank_rows.iloc[0]["Rank"]) if not rank_rows.empty else 0
    leader_elo = float(rankings.iloc[0]["Oracle Elo"]) if not rankings.empty else current["oracle"]
    gap = leader_elo - current["oracle"]
    streak_label = f"W{state.streak}" if state.streak > 0 else f"L{abs(state.streak)}" if state.streak < 0 else "—"
    profile_html = (
        f'<div class="profile-hero"><div class="eyebrow">Team intelligence · world rank #{rank}</div>'
        f'<div class="profile-name">{team}</div>'
        f'<div class="profile-sub">Oracle Elo {current["oracle"]:.0f} · {form["status"]} · {gap:.0f} points behind #1</div>'
        f'<div><span class="stat-chip">Peak {state.peak_oracle:.0f}</span>'
        f'<span class="stat-chip">{state.matches} rated matches</span>'
        f'<span class="stat-chip">Streak {streak_label}</span>'
        f'<span class="stat-chip">Rust {rest_days(state, now):.0f} days</span>'
        f'<span class="stat-chip">Certainty {100 * current["certainty"]:.0f}%</span>'
        f'<span class="stat-chip">Reset risk {100 * current["reset_risk"]:.0f}%</span></div></div>'
    )
    st.markdown(profile_html, unsafe_allow_html=True)
    m1, m2, m3, m4, m5, m6 = st.columns(6)
    m1.metric("Oracle Elo", f"{current['oracle']:.0f}")
    m2.metric("Core Elo", f"{current['core']:.0f}")
    m3.metric("Fast Elo", f"{current['fast']:.0f}", delta=f"{form['momentum30']:+.0f} / 30d")
    m4.metric("Heat", f"{form['heat']:.0f}/100", delta=form["status"])
    m5.metric("Certainty", f"{100 * current['certainty']:.0f}%", delta=f"Fast weight {100 * current['fast_weight']:.0f}%")
    m6.metric("Reset risk", f"{100 * current['reset_risk']:.0f}%", delta=f"Opposition {form['opponent_quality']:.0f}")
    st.markdown(recent_results_html(bundle, team, 12), unsafe_allow_html=True)

    left, right = st.columns([1.55, 1])
    with left:
        st.markdown("#### Elo trajectory")
        render_plotly(elo_chart(bundle, team, height=430), stable_key("profile_elo", team))
    with right:
        st.markdown("#### Rating DNA")
        render_plotly(rating_profile_chart(state, now), stable_key("profile_dna", team))

    left2, right2 = st.columns([1.45, 1])
    with left2:
        st.markdown("#### Recent form pulse")
        render_plotly(form_pulse_chart(bundle, team, height=310), stable_key("profile_form", team))
    with right2:
        st.markdown("#### Context performance · last 365 days")
        context = format_context_table(state, now)
        st.dataframe(context, hide_index=True, use_container_width=True,
                     column_config={"Win rate": st.column_config.ProgressColumn(format="%.1f%%", min_value=0, max_value=100)})

    team_history = team_history_frame(bundle, team)
    st.markdown("#### Recent Elo ledger")
    if team_history.empty:
        st.info("No detailed ledger rows are available.")
    else:
        display = team_history.tail(35).sort_values("date", ascending=False).copy()
        display["Expected win %"] = 100 * display["expected"]
        ledger_columns = ["date", "opponent", "event", "result", "score", "before", "after", "delta", "k", "margin_multiplier", "uncertainty", "reset_risk", "Expected win %"]
        ledger_columns = [column for column in ledger_columns if column in display.columns]
        st.dataframe(display[ledger_columns],
        ...
        with impact_cols[0]:
            st.markdown("##### Biggest recent Elo gains")
            gains = team_history.tail(80).nlargest(5, "delta")[["date", "opponent", "score", "delta"]]
            st.dataframe(gains, hide_index=True, use_container_width=True)
        with impact_cols[1]:
            st.markdown("##### Most damaging recent losses")
            losses = team_history.tail(80).nsmallest(5, "delta")[["date", "opponent", "score", "delta"]]
            st.dataframe(losses, hide_index=True, use_container_width=True)

    upcoming_team = upcoming[(upcoming["team_a"] == team) | (upcoming["team_b"] == team)].copy() if not upcoming.empty else pd.DataFrame()
    st.markdown("#### Upcoming fixtures")
    if upcoming_team.empty:
        st.caption("No upcoming fixture was returned in the current seven-day window.")
    else:
        upcoming_team["Start"] = upcoming_team["date"].dt.tz_convert(display_timezone)
        upcoming_team["Opponent"] = np.where(upcoming_team["team_a"] == team, upcoming_team["team_b"], upcoming_team["team_a"])
        st.dataframe(upcoming_team[["Start", "Opponent", "event", "best_of"]].rename(columns={"event": "Event", "best_of": "BO"}), hide_index=True, use_container_width=True,
                     column_config={"Start": st.column_config.DatetimeColumn(format="DD MMM HH:mm")})

    st.markdown("#### Current PandaScore roster")
    team_id = team_id_lookup.get(team)
    if not team_id:
        st.caption("A stable PandaScore team ID was not available for this profile.")
    else:
        try:
            players = team_players(cached_team(token, team_id))
            if players.empty:
                st.caption("The team endpoint did not return a current roster.")
            else:
                st.dataframe(players, hide_index=True, use_container_width=True)
        except APIError as exc:
            st.caption(f"Roster request unavailable: {exc}")


# ----------------------------- App shell -----------------------------
st.markdown(f"""
<div class="hero"><div class="eyebrow">CS Oracle · Drift-Aware Precision Engine {APP_VERSION}</div>
<h1>World-class CS2 team intelligence</h1>
<p>Drift-aware daily predictions, adaptive Elo, rating uncertainty, probable roster-reset protection, specialist temporal models and calibrated exact-score forecasts. Team profiles remain fully clickable and duplicate-safe.</p></div>
...
with st.sidebar:
    ...
    display_timezone = st.selectbox("Display timezone", timezone_options, index=0, key="display_timezone")
    st.caption(f"Today is {local_now(display_timezone).strftime('%A, %d %B %Y')} in {display_timezone}.")
    st.markdown("### Accuracy settings")
    history_days = st.slider("History window (days)", 420, 1100, DEFAULT_HISTORY_DAYS, 30, key="history_days")
    history_pages = st.slider("Maximum API pages", 12, 32, DEFAULT_HISTORY_PAGES, 1, key="history_pages")
    ...
    map_upload = st.file_uploader("Map history CSV", type=["csv"], help="Optional. Enables projected veto and map-specific context.", key="map_history_upload")
    player_upload = st.file_uploader("3-month player form CSV", type=["csv"], help="Optional columns: team, player, rating, adr, kast, impact, maps.", key="player_form_upload")
...
history, upcoming, team_aliases, team_id_lookup = canonicalise_team_identity(history, upcoming)
if not player_data.empty and team_aliases:
    player_data = player_data.copy()
    player_data["team"] = player_data["team"].map(lambda value: team_aliases.get(normalise_team(value), normalise_team(value)))
...
if not map_history.empty and team_aliases:
    for column in ["team_a", "team_b", "winner"]:
        if column in map_history.columns:
            map_history[column] = map_history[column].map(lambda value: team_aliases.get(normalise_team(value), normalise_team(value)))
...
with daily_tab:
    ...
    if bundle.metrics:
        st.caption(f"Well-covered BO3/BO5 test accuracy: {100 * bundle.metrics.get('quality_match_accuracy', 0):.1f}% over {bundle.metrics.get('quality_match_count', 0)} matches · strongest learned signals: {100 * bundle.metrics.get('strong_signal_accuracy', 0):.1f}% over {bundle.metrics.get('strong_signal_count', 0)} matches.")
    if bundle.winner_model is None:
    ...
    if upcoming.empty:
        ...
        scope = st.radio("Show matches", ["Today", "Tomorrow", "Next 3 days", "All loaded", "Choose date"], index=0, horizontal=True, key="daily_scope")
        chosen_date = None
        if scope == "Choose date":
            ...
            chosen_date = st.date_input("Match date", value=default_date, min_value=min(available_dates) if available_dates else today_local, max_value=max(available_dates) if available_dates else today_local, key="chosen_match_date")
        if scope == "Today":
        ...
        if scoped_upcoming.empty:
            ...
            with f1:
                filter_mode = st.radio("Prediction filter", ["All matches", "Strong + Elite", "Elite only"], index=0, horizontal=True, key="daily_prediction_filter")
            with f2:
                sort_mode = st.selectbox("Sort ranking by", ["Best overall signal", "Highest win chance", "Earliest start", "Latest start"], index=0, key="daily_sort")
            ...
            st.download_button("Download shown predictions", shown.to_csv(index=False).encode(), f"cs_oracle_{filename_date}.csv", "text/csv", use_container_width=True, key="download_daily_predictions")
...
with match_tab:
    if upcoming.empty:
        ...
        selected_label = st.selectbox("Choose match", labels, key="match_laboratory_selector")
        ...
        with left:
            ...
            render_plotly(fig, stable_key("match_probability", selected["match_id"]))
        with right:
        ...
        for col, team in zip(chart_cols, [selected["team_a"], selected["team_b"]]):
            with col:
                ...
                render_plotly(elo_chart(bundle, team, height=270, compact=True), stable_key("match_elo", selected["match_id"], team))
                ...
                render_plotly(form_pulse_chart(bundle, team, height=220), stable_key("match_form", selected["match_id"], team))
        ...
        st.download_button("Download locked prediction receipt", json.dumps(receipt, indent=2).encode(), f"prediction_{selected['match_id']}.json", "application/json", use_container_width=True, key=stable_key("download_receipt", selected["match_id"]))

with elo_tab:
    st.markdown("### Oracle Elo world ranking")
    st.caption("Click any team to open its full intelligence profile. Public Elo blends 66% stable Core Elo and 34% Fast Elo; match forecasts also use LAN/online and BO-format ratings.")
    rankings = elo_table(bundle)
    if not rankings.empty:
        now_utc = pd.Timestamp.now(tz="UTC")
        active_cutoff = now_utc - pd.Timedelta(days=120)
        rankings["Active"] = pd.to_datetime(rankings["Last match"], utc=True, errors="coerce") >= active_cutoff

        q1, q2, q3 = st.columns([1.5, 1, 1])
        with q1:
            team_search = st.text_input("Find a team", placeholder="Spirit, Vitality, NAVI…", key="elo_team_search")
        with q2:
            ranking_view = st.selectbox("Ranking view", ["Active teams", "All rated teams", "Super hot", "Rising fast"], key="elo_ranking_view")
        with q3:
            ranking_limit = st.selectbox("Rows", [15, 25, 40, 60], index=1, key="elo_ranking_limit")

        shown_rankings = rankings.copy()
        if ranking_view == "Active teams":
            shown_rankings = shown_rankings[shown_rankings["Active"]]
        elif ranking_view == "Super hot":
            shown_rankings = shown_rankings[shown_rankings["Heat"] >= 74].sort_values(["Heat", "Oracle Elo"], ascending=False)
        elif ranking_view == "Rising fast":
            shown_rankings = shown_rankings.sort_values(["30d change", "Heat"], ascending=False)
        if team_search.strip():
            shown_rankings = shown_rankings[shown_rankings["Team"].str.contains(team_search.strip(), case=False, regex=False)]
        clicked_team = render_clickable_elo_list(shown_rankings, int(ranking_limit))
        if clicked_team:
            st.session_state["elo_selected_team"] = clicked_team
        if "elo_selected_team" not in st.session_state or st.session_state["elo_selected_team"] not in bundle.states:
            st.session_state["elo_selected_team"] = str(rankings.iloc[0]["Team"])

        st.download_button("Download full Elo table", rankings.drop(columns=["Active"]).to_csv(index=False).encode(), "cs_oracle_elo_v8.csv", "text/csv", use_container_width=True, key="download_elo_table")

        st.markdown("---")
        team_profile_panel(bundle, rankings, str(st.session_state["elo_selected_team"]), upcoming, token, team_id_lookup, display_timezone)

        st.markdown("### 🔥 Hottest teams right now")
        hot = rankings.sort_values(["Heat", "30d change"], ascending=False).head(20).copy()
        st.dataframe(hot[["Rank", "Team", "Heat", "Form", "30d change", "60d change", "Last 10 win %", "Recent opponent Elo", "Oracle Elo"]],
                     hide_index=True, use_container_width=True,
                     column_config={"Heat": st.column_config.ProgressColumn(format="%.0f", min_value=0, max_value=100),
                                    "Last 10 win %": st.column_config.ProgressColumn(format="%.1f%%", min_value=0, max_value=100)})

        st.markdown("### Compact charts for leading teams")
        card_count = st.slider("Number of leading-team charts", 3, 12, 6, 1, key="leading_team_chart_count")
        for _, row in rankings.head(card_count).iterrows():
            team_name = str(row["Team"])
            with st.expander(f"#{int(row['Rank'])} {team_name} · {row['Form']} · Elo {row['Oracle Elo']:.0f}"):
                ...
                render_plotly(elo_chart(bundle, team_name, compact=True), stable_key("leader_elo", team_name))
                st.markdown(recent_results_html(bundle, team_name), unsafe_allow_html=True)
                render_plotly(form_pulse_chart(bundle, team_name, height=210), stable_key("leader_form", team_name))

        st.markdown("### How every Elo move is calculated")
        st.markdown('<div class="formula">Expected = 1 / (1 + 10 ^ (−Elo difference / 400))<br>K = 27 × event-tier multiplier × experience multiplier × score-margin multiplier<br>Core change = K × (actual result − expected result)<br>Fast Elo uses a larger K and regresses faster toward 1500<br>Oracle match Elo uses adaptive Core/Fast weights + 12% LAN/Online + 9% BO format<br>Fast-form weight rises automatically when uncertainty, inactivity or reset risk is high<br>Public ranking Elo also uses adaptive current-form weight and shrinks uncertain ratings toward 1500</div>', unsafe_allow_html=True)
        st.caption("Team IDs are canonicalised before training, so renamed organisations keep one continuous Elo history. A Glicko-style rating-deviation layer grows during inactivity, and probable roster resets shift weight from stale Core Elo toward Fast Elo. Heat combines 30/60-day Elo momentum, last-10 results, opponent-adjusted performance, opposition quality and rust.")
    else:
...
with accuracy_tab:
    ...
    if not bundle.metrics:
        ...
        st.caption(f"Oldest 68% train the models, the next 17% choose specialist recency, calibration and the precision gate, and the newest {metrics['test_matches']:,} matches stay untouched until final testing. Winner engine: {metrics['winner_model_name']} · calibration: {metrics.get('calibrator', '—')} · confidence shrink: {metrics.get('probability_shrink', 1.0):.2f}.")
        qa, qb = st.columns(2)
        qa.metric("Well-covered BO3/BO5 accuracy", f"{100 * metrics.get('quality_match_accuracy', 0):.1f}%", delta=f"{metrics.get('quality_match_count', 0)} untouched matches")
        qb.metric("Strongest learned signals", f"{100 * metrics.get('strong_signal_accuracy', 0):.1f}%", delta=f"{metrics.get('strong_signal_count', 0)} untouched matches")
...
with method_tab:
    st.markdown("### Why Version 9 is the strongest current-data model")
    st.markdown("""
- **Nine temporal specialists:** ultra-recent, balanced, long-memory, BO1, BO3, LAN, elite-event and lower-tier experts compete on chronological validation. The stacker learns which expert matters in each context.
- **Leak-resistant time weighting:** every historical fit calculates recency relative to its own cutoff, not a future date.
- **Drift-aware stacking:** reliable Elo, Fast/Core divergence, rating certainty, probable roster-reset risk, opponent-adjusted quality form, trend acceleration, event strength, model disagreement, sample depth, match tier, LAN/online context and BO format enter the final stack.
...
- **Adaptive strength ratings:** stable PandaScore identities are combined with Glicko-style uncertainty, inactivity growth, dynamic K-factors, field-strength-adjusted tournament importance and automatic Core-to-Fast reweighting when a team appears rebuilt or structurally changed.
- **One-click match-day board:** Today, Tomorrow, Next 3 days or a chosen local date can be ranked independently.
- **Clickable team intelligence:** every ranking row opens a full profile with Elo history, rating DNA, context splits, roster, upcoming fixtures and the complete calculation ledger.
- **Duplicate-safe dashboard:** deterministic keys are assigned to every dynamic chart and widget, eliminating cross-tab `StreamlitDuplicateElementId` failures.
...
st.caption("CS Oracle Drift-Aware Precision uses no bookmaker feed. It provides probabilistic estimates, not certainty. Never treat a prediction as guaranteed. Regenerate any API token that has been shared publicly.")
