...
odds_debug: dict[str, Any] = {"events": 0, "paired": 0, "payloads": 0, "bookmakers": []}
recommended_bookmakers = ["Bet365", "GG.BET"]
...
if odds_token and not secret_bookmakers and not session_bookmakers:
    try:
...
odds_bookmakers = (secret_bookmakers or session_bookmakers or account_bookmakers or recommended_bookmakers)[:2]
using_recommended_bookmakers = bool(odds_token and not (secret_bookmakers or session_bookmakers or account_bookmakers))
...
def show_bookmaker_activation() -> None:
    ...
    selected = st.multiselect(
        "Bookmakers",
        ["Bet365", "GG.BET", "Thunderpick", "Unibet", "Betway", "1xBet", "Stake"],
        default=odds_bookmakers[:2],
        max_selections=2,
        key="odds_bookmaker_setup",
    )
    if st.button("Activate odds feed", type="primary", key="activate_odds_feed"):
        try:
            activated = OddsClient(odds_token).select_bookmakers(selected)
            st.session_state["odds_selected_bookmakers"] = activated or selected
            cached_selected_bookmakers.clear()
            cached_odds_events.clear()
            cached_odds_payloads.clear()
            st.success("Bookmakers activated: " + ", ".join(activated or selected))
            st.rerun()
        except OddsAPIError as exc:
    ...
    try:
        with st.spinner("Matching fixtures to bookmakers and fetching current match-winner odds…"):
            ...
            if not odds_events:
                ...
                if using_recommended_bookmakers:
                    show_bookmaker_activation()
    except OddsAPIError as exc:
        odds_status = "Feed error"
        st.warning(f"Predictions still work, but odds could not load: {exc}")
        if using_recommended_bookmakers or "bookmaker" in str(exc).lower() or "403" in str(exc):
            show_bookmaker_activation()
elif not odds_token:
...
with daily_tab:
    ...
    if odds_token:
        st.caption(
            f"Odds diagnostics · books: {', '.join(odds_debug.get('bookmakers', [])) or 'none'} · "
