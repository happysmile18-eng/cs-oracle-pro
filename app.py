...
APP_VERSION = "8.0.0-team-intelligence"
...
st.set_page_config(page_title="CS Oracle Team Intelligence", page_icon="🎯", layout="wide", initial_sidebar_state="collapsed")
...
def team_profile_panel(bundle: ModelBundle, rankings: pd.DataFrame, team: str, upcoming: pd.DataFrame, token: str, team_id_lookup: dict[str, int], display_timezone: str) -> None:
    ...
    profile_html = (
        ...
        f'<span class="stat-chip">Rust {rest_days(state, now):.0f} days</span></div></div>'
    )
    ...
    m5.metric("Last 10", f"{100 * form['wr10']:.0f}%")
    m6.metric("Opposition", f"{form['opponent_quality']:.0f} Elo")
...
# ----------------------------- App shell -----------------------------
st.markdown(f"""
<div class="hero"><div class="eyebrow">CS Oracle · Team Intelligence Engine {APP_VERSION}</div>
<h1>World-class CS2 team intelligence</h1>
<p>Daily predictions, clickable team profiles, identity-corrected Elo history, hot-form intelligence and calibrated exact-score forecasts. Every dynamic chart and widget has a deterministic ID, keeping all tabs stable across reruns.</p></div>
...
with elo_tab:
    ...
    if not rankings.empty:
        ...
        st.caption("Team IDs are canonicalised before training, so renamed organisations keep one continuous Elo history. Inactive ratings regress gradually toward 1500. Heat combines 30/60-day Elo momentum, last-10 results, opponent-adjusted performance, opposition quality and rust.")
    else:
...
- **Stable team identity:** PandaScore team IDs unify renamed organisations before training, preventing split Elo histories and lost form.
