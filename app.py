...
APP_VERSION = "5.3.0-free-books-auto"
...
# Official Odds-API.io changelog examples of recreational books included on the free plan.
# The app only attempts these names and never offers sharp/exchange books on a free key.
FREE_RECREATIONAL_BOOKMAKERS = ["Bet365", "Unibet", "DraftKings", "FanDuel"]
...
class OddsClient:
    ...
    def odds_for_events(self, event_ids: list[str], bookmakers: list[str]) -> list[dict[str, Any]]:
        """Fetch odds defensively, one bookmaker at a time.

        A single inaccessible sharp/exchange book must never kill the whole feed.
        Free-plan recreational books are attempted independently in batches of ten
        events; unavailable books are skipped and successful responses are merged.
        """
        books = []
        seen: set[str] = set()
        ...
        books = books[:8]
        if not books:
            raise OddsAPIError("No free recreational bookmakers are configured.")
        ...
        for book in books:
            ...
            for start in range(0, len(event_ids), 10):
                ...
                try:
                    payload = self._get(
                        "/odds/multi",
                        {"eventIds": ",".join(batch), "bookmakers": book},
                    )
                    rows = self._rows(payload)
                    if rows:
                    ...
                    book_errors.append(str(exc))

                # Some feeds reject a mixed batch when one event is missing.
                # Retry each event so valid prices are still recovered.
                for event_id in batch:
                    try:
                        rows = self._rows(
                            self._get("/odds", {"eventId": event_id, "bookmakers": book})
                        )
                        book_rows.extend(rows)
                    except OddsAPIError as exc:
                        book_errors.append(str(exc))
            ...
            if book_rows:
                ...
                output.extend(book_rows)
            elif book_errors:
                errors.append(f"{book}: {book_errors[-1]}")

        # Deduplicate exact API objects while preserving separate bookmaker quotes.
        ...
        if event_ids and not deduped and errors:
            preview = " | ".join(errors[:3])
            raise OddsAPIError(
                "No free recreational bookmaker returned prices. " + preview
            )
        return deduped
...
def canonical_bookmaker_names(candidates: list[str], catalogue: list[str]) -> list[str]:
    """Return valid catalogue names, preserving the API's exact spelling."""
...
# Never ask a free-plan user to choose from the full catalogue: that list also
# contains sharp books, exchanges and prediction markets that return HTTP 403.
# Restrict the feed to officially documented recreational examples and let the
# odds client test each independently so one unavailable source cannot break all.
...
if odds_token:
    ...
    if available_bookmakers:
        odds_bookmakers = canonical_bookmaker_names(
            FREE_RECREATIONAL_BOOKMAKERS, available_bookmakers
        )
    else:
        odds_bookmakers = list(FREE_RECREATIONAL_BOOKMAKERS)

    # Bet365 and Unibet are the first choices for international CS2 coverage.
    # The remaining documented recreational books are automatic fallbacks.
    odds_bookmakers = odds_bookmakers[:4]

    # Best-effort account setup. Failure is harmless because direct odds requests
    # below still test every free book independently.
    try:
        saved = cached_selected_bookmakers(odds_token)
        saved_free = canonical_bookmaker_names(saved, odds_bookmakers)
        if not saved_free and len(odds_bookmakers) >= 2:
            try:
                OddsClient(odds_token).select_bookmakers(odds_bookmakers[:2])
...
if odds_token and odds_bookmakers and not upcoming.empty:
    odds_debug["bookmakers"] = odds_bookmakers
    try:
        with st.spinner("Matching fixtures and fetching free-plan bookmaker prices…"):
            ...
            payloads = cached_odds_payloads(
                odds_token, event_ids, tuple(odds_bookmakers)
            ) if event_ids else []
            ...
            if not odds_events:
                st.warning("Odds-API.io returned no esports events in this date range.")
            elif not pairings:
                st.warning(
                    f"Found {len(odds_events)} esports events, but none safely matched the PandaScore fixtures. "
                    "Many low-tier CS2 matches have no bookmaker coverage."
                )
            elif not market_by_match:
                st.warning(
                    f"Matched {len(pairings)} fixtures, but the free recreational books "
                    f"({', '.join(odds_bookmakers)}) returned no two-way winner prices."
                )
    except OddsAPIError as exc:
        odds_status = "Feed error"
        st.warning(f"Predictions still work, but free bookmaker odds could not load: {exc}")
        st.caption(
            "The app now excludes sharp/exchange books automatically. If this remains, "
            "the covered CS2 fixtures may simply have no prices on the free recreational feeds."
        )
elif not odds_token:
...
""")
    ...
    st.warning("PandaScore Free does not automatically provide complete three-month ADR, player ratings or map pick/ban history. Odds coverage depends on the recreational bookmakers available on the free Odds-API.io plan. Team-name and fixture matching is scored and weak matches are rejected rather than forced.")
