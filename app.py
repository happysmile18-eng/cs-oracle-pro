...
APP_VERSION = "6.0.0-pure-prediction"
    ...
    if len(history) >= MIN_MODEL_MATCHES:
        ...
        if train_mask.sum() >= 300 and len(calib_indices) >= 95 and test_mask.sum() >= 80:
            ...
            for candidate_i, config in enumerate(WINNER_MODEL_CONFIGS):
                ...
                score = float(brier_score_loss(stack_y, p) + 0.10 * log_loss(stack_y, p, labels=[0, 1]))
...
# ----------------------------- App shell -----------------------------
st.markdown(f"""
<div class="hero"><div class="eyebrow">CS Oracle · Pure Prediction Engine {APP_VERSION}</div>
<h1>Pure CS2 prediction intelligence</h1>
<p>No bookmaker or odds dependency. Three differently paced temporal XGBoost models are stacked with Oracle Elo and form intelligence, then calibrated on later matches. The engine ranks every fixture, predicts the exact score, and only uses a 70% label when untouched history proves it.</p></div>
...
try:
    with st.spinner("Loading completed CS2 matches and rebuilding the Pure Prediction Engine…"):
...
with daily_tab:
    ...
    if upcoming.empty:
        ...
        if filter_mode == "Elite only":
            shown = board[board["Signal"] == "Elite 70% proven"]
        elif filter_mode == "Strong + Elite":
            shown = board[board["Signal"].isin(["Strong signal", "Elite 70% proven"])]
        else:
            shown = board
...
with method_tab:
    ...
    st.markdown("""
- **Three temporal experts:** fast-form, balanced and long-memory XGBoost models compete and the strongest three are stacked instead of trusting one fragile fit.
- **Leak-resistant time weighting:** every historical fit calculates recency relative to its own cutoff, not a future date.
- **Independent Elo and form views:** Oracle Elo, Fast Elo and opponent-adjusted form enter the final stack as separate evidence.
...
st.caption("CS Oracle Pure Prediction uses no bookmaker feed. It provides probabilistic estimates, not certainty. Never treat a prediction as guaranteed. Regenerate any API token that has been shared publicly.")
