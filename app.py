...
# ----------------------------- App shell -----------------------------
st.markdown(f"""
<div class="hero"><div class="eyebrow">CS Oracle · Precision Engine {APP_VERSION}</div>
<h1>Winner precision + exact score</h1>
<p>A dedicated calibrated winner model is optimised for picking the right team. A second conditional score head estimates 2–0, 2–1 and other exact outcomes. A learned precision gate may abstain from weak matches and only marks a 70% signal when the newest untouched backtest proves it.</p></div>
...
try:
    with st.spinner("Loading completed CS2 matches and rebuilding Precision Engine…"):
...
with st.spinner("Training winner model, exact-score head, precision gate and Elo history…"):
...
with daily_tab:
    ...
    if upcoming.empty:
        ...
        board = pd.DataFrame(rows).sort_values(["Precision score", "Win probability"], ascending=False)
        only_elite = st.toggle("Show only Elite 70% signals", value=False, disabled=not bool(bundle.metrics.get("precision_gate_target_met", False)))
        shown = board[board["Signal"] == "Elite 70% proven"] if only_elite else board
        ...
        st.download_button("Download daily predictions", board.to_csv(index=False).encode(), "cs_oracle_precision_daily.csv", "text/csv", use_container_width=True)
...
with accuracy_tab:
    ...
    if not bundle.metrics:
        ...
        st.caption(f"Oldest 70% train the models, the next 14% tune calibration and the precision gate, and the newest {metrics['test_matches']:,} matches stay untouched until final testing. Winner model: {metrics['winner_model_name']}.")
...
with method_tab:
    ...
    st.markdown("""
- **Winner-first architecture:** a binary XGBoost head is tuned specifically for choosing the winning team instead of making winner accuracy a side effect of multiclass score prediction.
- **Conditional exact score:** a separate score head learns 2–0, 2–1 and other outcomes, then its winner total is forced to match the better calibrated winner head.
- **Automatic model selection:** three regularised XGBoost structures compete on a later chronological calibration period; only the best probability model is retained.
- **Platt calibration:** raw winner probabilities are corrected on later matches before touching the newest test period.
- **Learned precision gate:** a small reliability model studies confidence, Elo agreement, symmetry, sample size, tier and form separation. It can abstain from low-information matches.
- **No fake 70% promise:** the Elite label is enabled only when the newest untouched test period actually reaches at least 70% with a meaningful sample.
- **Hot-form intelligence:** Core, Fast and Oracle Elo histories are graphed; teams are labelled Super hot, Hot, Stable, Cooling or Cold.
""")
    st.markdown("### Why no LLM or neural network is the main predictor")
    st.info("An LLM is useful for summarising roster news, but it does not create better numeric probabilities from this small structured dataset. Deep neural networks normally need much richer map, round and player telemetry. Regularised gradient boosting is the stronger choice for the data currently available.")
