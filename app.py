...
APP_VERSION = "5.2.0-dynamic-bookmakers"
...
def odds_bookmakers_from_secrets() -> list[str]:
    try:
        raw = str(st.secrets.get("ODDS_BOOKMAKERS", "")).strip()
    except Exception:
        raw = ""
    return [item.strip() for item in raw.split(",") if item.strip()][:30]
...
def _extract_bookmaker_names(payload: Any) -> list[str]:
    """Parse the account's selected-bookmaker response across known API shapes."""
    names: list[str] = []

    def add(value: Any) -> None:
        if isinstance(value, str):
            value = value.strip()
            if value and value.lower() not in {"success", "ok", "selected"}:
                names.append(value)
        elif isinstance(value, list):
            for item in value:
                add(item)
        elif isinstance(value, dict):
            for key in ("name", "title", "bookmaker"):
...
class OddsClient:
    ...
    def _request(self, method: str, path: str, params: dict[str, Any] | None = None) -> Any:
        ...
        for attempt in range(4):
            ...
            if response.status_code == 429:
                ...
                last_error = f"Odds-API.io rate limit reached" + (f" (reset: {reset})" if reset else "")
                time.sleep(1.2 + 1.2 * attempt)
                continue
            if response.status_code in (401, 403):
                detail = response.text[:240]
                raise OddsAPIError(f"Odds-API.io returned {response.status_code}. Check ODDS_API_KEY and bookmaker access. {detail}")
            if response.status_code >= 400:
                raise OddsAPIError(f"Odds-API.io returned {response.status_code}: {response.text[:260]}")
            try:
                return response.json()
            except ValueError as exc:
                raise OddsAPIError(f"Odds-API.io returned invalid JSON: {exc}") from exc
    ...
    def available_bookmakers(self) -> list[str]:
        """Fetch the live canonical bookmaker names accepted by Odds-API.io.

        The endpoint is public. We intentionally do not rely on hard-coded bookmaker
        names because branding and accepted API names can change.
        """
        url = f"{ODDS_API_ROOT}/bookmakers"
        last_error = "Could not load the Odds-API.io bookmaker catalogue."
        for attempt in range(4):
            try:
                response = requests.get(url, timeout=self.timeout, headers={"Accept": "application/json"})
            except requests.RequestException as exc:
                last_error = f"Bookmaker catalogue connection failed: {exc}"
                time.sleep(0.7 + attempt)
                continue
            if response.status_code >= 400:
                last_error = f"Bookmaker catalogue returned {response.status_code}: {response.text[:220]}"
                time.sleep(0.7 + attempt)
                continue
            try:
                payload = response.json()
            except ValueError as exc:
                last_error = f"Bookmaker catalogue returned invalid JSON: {exc}"
                time.sleep(0.7 + attempt)
                continue
            rows = payload if isinstance(payload, list) else self._rows(payload)
            names: list[str] = []
            for row in rows:
            ...
            last_error = "Odds-API.io returned an empty bookmaker catalogue."
        raise OddsAPIError(last_error)

    def selected_bookmakers(self) -> list[str]:
        return _extract_bookmaker_names(self._get("/bookmakers/selected"))

    def select_bookmakers(self, bookmakers: list[str]) -> list[str]:
        cleaned = [str(item).strip() for item in bookmakers if str(item).strip()][:2]
        if not cleaned:
            raise OddsAPIError("Choose at least one bookmaker.")
        self._request("PUT", "/bookmakers/selected/select", {"bookmakers": ",".join(cleaned)})
        selected = self.selected_bookmakers()
        return selected or cleaned

    def events(self, date_from: str, date_to: str) -> list[dict[str, Any]]:
        # Odds-API.io returns only the next 14 days by default. We provide an explicit
        # range matching the PandaScore board and paginate so CS2 is not hidden behind
        # other esports titles in a small first page.
        output: list[dict[str, Any]] = []
        limit = 500
        for skip in range(0, 5000, limit):
            payload = self._get(
                "/events",
                {
                    "sport": "esports",
    ...
    def odds_for_events(self, event_ids: list[str], bookmakers: list[str]) -> list[dict[str, Any]]:
        books = [str(item).strip() for item in bookmakers if str(item).strip()][:30]
        if not books:
            raise OddsAPIError("No Odds-API.io bookmakers are selected. Select up to two bookmakers on the free plan.")

        def fetch_with(book_list: list[str]) -> tuple[list[dict[str, Any]], list[str]]:
            output: list[dict[str, Any]] = []
            errors: list[str] = []
            book_param = ",".join(book_list)
            for start in range(0, len(event_ids), 10):
                batch = event_ids[start:start + 10]
                if not batch:
                    continue
                try:
                    payload = self._get("/odds/multi", {"eventIds": ",".join(batch), "bookmakers": book_param})
                    rows = self._rows(payload)
                    if rows:
                        output.extend(rows)
                        continue
                ...
                # Single-event fallback gives clearer behaviour if one bad event breaks a batch.
                for event_id in batch:
                    try:
                        output.extend(self._rows(self._get("/odds", {"eventId": event_id, "bookmakers": book_param})))
                    except OddsAPIError as exc:
                        errors.append(str(exc))
            return output, errors

        output, errors = fetch_with(books)
        # Defensive recovery: a bookmaker can be renamed or unavailable between the
        # catalogue request and the odds request. If the combined request fails, retry
        # each selected bookmaker independently so one stale name cannot kill the feed.
        if not output and len(books) > 1:
            for book in books:
                rows, book_errors = fetch_with([book])
                output.extend(rows)
                errors.extend(book_errors)

        if event_ids and not output and errors:
            raise OddsAPIError(errors[-1])
        return output



@st.cache_data(ttl=6 * 3600, max_entries=4, show_spinner=False)
def cached_available_bookmakers() -> list[str]:
    # A placeholder API key is fine because this endpoint is public and the client
    # method does not send authentication.
    return OddsClient("public").available_bookmakers()


@st.cache_data(ttl=900, max_entries=8, show_spinner=False)
def cached_selected_bookmakers(api_key: str) -> list[str]:
    return OddsClient(api_key).selected_bookmakers()


@st.cache_data(ttl=420, max_entries=8, show_spinner=False)
def cached_odds_events(api_key: str, date_from: str, date_to: str) -> list[dict[str, Any]]:
    return OddsClient(api_key).events(date_from, date_to)


@st.cache_data(ttl=60, max_entries=16, show_spinner=False)
def cached_odds_payloads(api_key: str, event_ids: tuple[str, ...], bookmakers: tuple[str, ...]) -> list[dict[str, Any]]:
    return OddsClient(api_key).odds_for_events(list(event_ids), list(bookmakers))
...
if not panda_token:
    ...
    st.code('PANDASCORE_TOKEN = "your-private-token"\nODDS_API_KEY = "your-private-odds-key"\n# Optional fallback if your Odds-API account has no saved selection:\nODDS_BOOKMAKERS = "Bet365,SingBet"', language="toml")
    st.stop()
...
odds_debug: dict[str, Any] = {"events": 0, "paired": 0, "payloads": 0, "bookmakers": [], "catalogue": 0}
secret_bookmakers = odds_bookmakers_from_secrets()
session_bookmakers = list(st.session_state.get("odds_selected_bookmakers", []))
account_bookmakers: list[str] = []
available_bookmakers: list[str] = []
bookmaker_catalogue_error = ""

if odds_token:
    try:
        available_bookmakers = cached_available_bookmakers()
        odds_debug["catalogue"] = len(available_bookmakers)
    except OddsAPIError as exc:
        bookmaker_catalogue_error = str(exc)
        # These names are documented examples and are used only if the live public
        # catalogue is temporarily unavailable. They are still validated by the API
        # when the user activates them.
        available_bookmakers = ["Bet365", "SingBet", "Unibet"]

    if not secret_bookmakers and not session_bookmakers:
        try:
            account_bookmakers = cached_selected_bookmakers(odds_token)
        except OddsAPIError as exc:
            st.warning(f"The odds key was found, but the saved bookmaker selection could not be read: {exc}")


def canonical_bookmaker_names(candidates: list[str], catalogue: list[str]) -> list[str]:
    """Return only valid catalogue names, preserving the API's exact spelling."""
    lookup = {name.casefold(): name for name in catalogue}
    output: list[str] = []
    seen: set[str] = set()
    for candidate in candidates:
        exact = lookup.get(str(candidate).strip().casefold())
...
preferred_order = ["Bet365", "SingBet", "Unibet", "Stake", "1xBet", "Betway", "Pinnacle", "Betfair", "William Hill"]
preferred_valid = canonical_bookmaker_names(preferred_order, available_bookmakers)
if len(preferred_valid) < 2:
    preferred_valid.extend([name for name in available_bookmakers if name not in preferred_valid][: 2 - len(preferred_valid)])

secret_valid = canonical_bookmaker_names(secret_bookmakers, available_bookmakers)
session_valid = canonical_bookmaker_names(session_bookmakers, available_bookmakers)
account_valid = canonical_bookmaker_names(account_bookmakers, available_bookmakers)
source_valid = secret_valid or session_valid or account_valid
odds_bookmakers = (source_valid or preferred_valid)[:2]
invalid_configured = [name for name in (secret_bookmakers + session_bookmakers + account_bookmakers) if name.casefold() not in {x.casefold() for x in available_bookmakers}]
needs_activation = bool(odds_token and (len(source_valid) < 2 or bool(invalid_configured)))
if "odds_bookmaker_setup" in st.session_state:
    stale_widget_values = [name for name in st.session_state.get("odds_bookmaker_setup", []) if name not in available_bookmakers]
    if stale_widget_values:
        del st.session_state["odds_bookmaker_setup"]


def show_bookmaker_activation() -> None:
    st.markdown("#### Activate your two free bookmakers")
    if bookmaker_catalogue_error:
        st.caption(f"The live bookmaker list could not be loaded, so a safe documented fallback is shown: {bookmaker_catalogue_error}")
    elif available_bookmakers:
        st.caption(f"Loaded {len(available_bookmakers)} currently accepted bookmaker names directly from Odds-API.io.")
    defaults = [name for name in odds_bookmakers if name in available_bookmakers][:2]
    if not defaults:
        defaults = preferred_valid[:2]
    selected = st.multiselect(
        "Bookmakers",
        available_bookmakers,
        default=defaults,
        max_selections=2,
        key="odds_bookmaker_setup",
    )
    if st.button("Activate odds feed", type="primary", key="activate_odds_feed", disabled=not selected):
        try:
            activated = OddsClient(odds_token).select_bookmakers(selected)
            final_selection = canonical_bookmaker_names(activated or selected, available_bookmakers) or selected
            st.session_state["odds_selected_bookmakers"] = final_selection[:2]
            cached_selected_bookmakers.clear()
            cached_odds_events.clear()
            cached_odds_payloads.clear()
            st.success("Bookmakers activated: " + ", ".join(final_selection[:2]))
            st.rerun()
        except OddsAPIError as exc:
            st.error(f"Could not activate those bookmakers: {exc}")


if invalid_configured:
    st.warning("Ignored invalid or outdated bookmaker name(s): " + ", ".join(sorted(set(invalid_configured))))


if odds_token and needs_activation:
    show_bookmaker_activation()

if odds_token and odds_bookmakers and not upcoming.empty and not needs_activation:
    odds_debug["bookmakers"] = odds_bookmakers
    try:
        with st.spinner("Matching fixtures to bookmakers and fetching current match-winner odds…"):
            board = upcoming.head(60)
            start_ts = board["date"].min() - pd.Timedelta(hours=8)
            end_ts = board["date"].max() + pd.Timedelta(days=2)
            date_from = start_ts.isoformat().replace("+00:00", "Z")
            date_to = end_ts.isoformat().replace("+00:00", "Z")
            odds_events = cached_odds_events(odds_token, date_from, date_to)
            odds_debug["events"] = len(odds_events)
            pairings = pair_odds_events(board, odds_events)
            odds_debug["paired"] = len(pairings)
            event_ids = tuple(sorted({str(row["event_id"]) for row in pairings.values()}))
            payloads = cached_odds_payloads(odds_token, event_ids, tuple(odds_bookmakers)) if event_ids else []
            odds_debug["payloads"] = len(payloads)
            ...
            if not odds_events:
                st.warning("Odds-API.io returned no esports events in the current board's date range.")
            elif not pairings:
                st.warning(f"Odds-API.io returned {len(odds_events)} esports events, but none matched the PandaScore fixtures. Many low-tier matches simply have no bookmaker coverage.")
            elif not market_by_match:
                st.warning(f"Matched {len(pairings)} fixtures, but no two-way match-winner prices were returned from {', '.join(odds_bookmakers)}.")
                if needs_activation:
                    st.info("Choose two valid bookmakers in the activation panel above, then rerun the app.")
    except OddsAPIError as exc:
        odds_status = "Feed error"
        st.warning(f"Predictions still work, but odds could not load: {exc}")
        if needs_activation or "bookmaker" in str(exc).lower() or "403" in str(exc):
            st.info("Use the bookmaker activation panel above. It now loads the accepted names directly from the API.")
elif not odds_token:
...
with daily_tab:
    ...
    if odds_token:
        st.caption(
            f"Odds diagnostics · catalogue: {odds_debug.get('catalogue', 0)} · books: {', '.join(odds_debug.get('bookmakers', [])) or 'none'} · "
            f"esports events: {odds_debug.get('events', 0)} · fixture matches: {odds_debug.get('paired', 0)} · "
            f"odds responses: {odds_debug.get('payloads', 0)}"
        )
